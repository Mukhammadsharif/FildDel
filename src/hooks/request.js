...
export function useRequest(options = {}) {
    ...
    const { token, signOut, lang } = useContext(GlobalContext)
    ...
    async function request(overrideOptions = {}, sync = false) {
        ...
        try {
            ...
            if (e.response === undefined) {
                ...
                typeof signOut === 'function' ? signOut(overrideOptions.navigation) : null
            }
...
export function useLoad(options, dependencies = []) {
    const { setNavigationRoute } = useContext(Context)
